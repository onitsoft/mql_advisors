   ...
   if(custom_high_filter)
     {
      if(fl_init && fl_percent)
        {
         GetHighParam(custom_high_range_hours,current_high_info);
   ...
   if(PositionSelect(_Symbol))
     {
      ...
      if(fl_init && fl_high && fl_percent && fl_margin)
        {
...
void OnTimer()
  {
   ...
   if(use_stop_loss)
     {
      execute_stop_loss(stop_loss_percent);
     }
...
double GetHighParam(int range,high_info &result_param)//range = kolvo 4asov
  {
   ...
   double current_diff=current_high-last_tick.last;
   if(current_diff>0)
     {
...
double GetDropModifier(double curr_high_diff)
  {
             return 0;
  }
//+------------------------------------------------------------------+
