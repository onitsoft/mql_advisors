...
void OnTick()
  {
   ...
   if(stime.hour==10 && stime.min==1 && stime.sec<5)
     {
      return;
     }
...
// Measure that the
//price change is acceptable according to our strategy @see README 
      ...
      if(PositionSelect(_Symbol))
        {
         ...
         curr_deposit*robot_amount_deposit_percent && lot_margin*3<=curr_deposit*robot_amount_deposit_percent)
           {
            fl_margin=true;
           }
        }
  ...
  void OnTradeTransaction(const MqlTradeTransaction &trans,
                           ...
                           const MqlTradeResult &result)
     {
      ...
      if(trans.order_state==ORDER_STATE_FILLED)
        {
         if(trans.order_type==ORDER_TYPE_BUY_LIMIT)///////BUY
            //LIMIT 
           {
            ...
            SetExecutionToRegistryOrder(NormalizeDouble(trans.price,_Digits),false,0);
         ...
         ///// END BUY LIMIT 
         else if(trans.order_type==ORDER_TYPE_SELL_LIMIT)///////SELL LIMIT 
           {
            ...
            SetExecutionToRegistryOrder(NormalizeDouble(trans.price,_Digits),true,0);
   ...
   double GetTargetPercentPrice(double price,double percentage,bool higher)//procenti ukazivautsya v izmenenii 
     {
      ...
      if(higher)
      ...
      else
        {
         prom_price=price-(price*percentage);
         double curr_point=SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE);
         if(curr_point!=0.0001 && curr_point!=0.001 && curr_point!=0.01
            && curr_point!=0.1 && curr_point!=1 && curr_point!=10)
           {prom_price=NormalizeDouble(prom_price,_Digits-1);}
         else
           {prom_price=NormalizeDouble(prom_price,_Digits);}
            ...
            if(res!=1111)
              {
               ...
               if(!PlaceOrder(adapted_percent_price,
                  ...
                  true,
                  0,
                  accumulate_registry[res].string_param
                  ))
                 {
...
//+------------------------------------------------------------------+ 
   ...
   void OnTimer()
     {
      ...
      // 
      if(stime.hour==18 && stime.min==39 && stime.sec<30)
        {SetNullForAccumulateRegistry();}
      // 
      DeleteExpiredRegistryOrders();
      GetAccumulateAverageParameteres(curr_last);
      if(recover_expired_daily_ordes){RecoverExpiredSellOrders();}
      //      DeleteUnstructedOrders(true);

      if(PositionSelect(_Symbol) && TimeCurrent()-last_registry_recover>360)
        {RegistryRecover();last_registry_recover=TimeCurrent();}
  ...
  int AddOrderToRegistry(double price_open,double volume,int expiration_sec,
                          ...
                          int cover_order_index=0,int lead_order_index=0,int OCOnumb=0) //+------------------------------------------------------------------+ 
     {
            ...
            if(accumulate_registry[x].price_open==0)
              {
               ...
               accumulate_registry[x].OCOnum=OCOnumb;
               accumulate_registry[x].string_param=StringGenerate(x);
   ...
   void GetAccumulateAverageParameteres(double price_last)//higher = true - zna4it vishe ceni, buy_true = lower = buy higher = sell 
     {
      ...
      double
      nearest_pending_sell_result_dev=0;
               ...
               if(TimeCurrent()-accumulate_registry[index].time<accumulate_registry[index].expiration_chain_sec && accumulate_registry[index].execution_strike_price!=0)
                 {
                  ...
                  int order_result=AddOrderToRegistry(NormalizeDouble(accumulate_registry[index].execution_strike_price,_Digits),
                                                      ...
                                                      accumulate_registry[index].time, 0, 0, index,accumulate_registry[index].OCOnum);
                  if(order_result!=1111)
                    {
                     if(PlaceOrder(NormalizeDouble(accumulate_registry[index].execution_strike_price,_Digits),
                        accumulate_registry[index].volume,
                        false,
                        true,
                        0,
                        accumulate_registry[order_result].string_param))
                       ...
                       {
                        Print("Failed sell init order on price ",accumulate_registry[index].execution_strike_price);
               ...
               if(TimeCurrent()-accumulate_registry[index].time<accumulate_registry[index].expiration_chain_sec && accumulate_registry[index].execution_strike_price!=0)
                 {
                  ...
                  int order_result=AddOrderToRegistry(NormalizeDouble(accumulate_registry[index].execution_strike_price,_Digits),
                                                      ...
                                                      accumulate_registry[index].time,0,0,0,OCOnum);

                  if(order_result!=1111)
                    {

                     if(!PlaceOrder(NormalizeDouble(accumulate_registry[index].execution_strike_price,_Digits),//add if
                        accumulate_registry[index].volume,
                        true,
                        true,
                        0,
                        accumulate_registry[order_result].string_param))
                       {
                        SetNullForAccumulateRegistry(order_result);
                       }
                    }
                 }
               if(accumulate_registry[index].lead_order_index!=1111)
   ...
   void DeleteExpiredRegistryOrders(bool stock_mode=true)
     {
      ...
      for(int a=0;a<size;a++)
        ...
        {
         fl_del=0;
         if(accumulate_registry[a].price_open!=0 &&
            accumulate_registry[a].is_executed==0 &&
            accumulate_registry[a].price_open!=1111)
           {
            ...
            if(accumulate_registry[a].expiration_chain_sec!=0)
              {
               if(accumulate_registry[a].ask_true==false && 
                  TimeCurrent()-accumulate_registry[a].time>accumulate_registry[a].expiration_chain_sec)
                 {fl_del=1;}
              }
         ...
         if(OrderSelect(accumulate_registry[a].ticket_open))
           {
            OrderDeleteAsync(accumulate_registry[a].ticket_open);
            Print("delete expired order on price ",accumulate_registry[a].price_open);
           }
         else
           {
            orders=OrdersTotal();
                                     ...
                                     int expirate_range_ticket=0)
     {
            ...
            for(int i=0;i<size;i++)
              {
               ...
               accumulate_registry[i].lead_order_index=1111;
               accumulate_registry[i].cover_order_index=1111;
               accumulate_registry[i].OCOnum=0;
               accumulate_registry[i].string_param="";
      ...
      //+--------------------------------------------------------+ 
      else
        {
         ...
         accumulate_registry[single_index].cover_order_index=1111;
         accumulate_registry[single_index].lead_order_index=1111;
         accumulate_registry[single_index].OCOnum=0;
         accumulate_registry[single_index].string_param="";
        }
     }
//+------------------------------------------------------------------+
//|                                                                  |
               ...
               if(!registry_mode)
                 {
                  if(OrderGetString(ORDER_SYMBOL)!=_Symbol){continue;}
                  double pr=OrderGetDouble(ORDER_PRICE_OPEN);
                  if(pr!=info_pos_ask.price_open &&
                     pr!=info_pos_ask_sub1.price_open &&
                     pr!=info_pos_ask_sub2.price_open &&
                     pr!=info_pos_bid.price_open &&
                     pr!=info_pos_bid_sub1.price_open &&
                     pr!=info_pos_bid_sub2.price_open)
                    {
                     OrderDeleteAsync(ticket);
   ...
   void RecoverExpiredSellOrders()
     {
      ...
      for(int i=0;i<size;i++)
        ...
        {
         if(accumulate_registry[i].is_executed==0 && 
            accumulate_registry[i].ask_true==true
            )
           {
            if(accumulate_registry[i].ask_true==true)
              {
               ...
               if(!IsActualOrder(accumulate_registry[i].price_open))
                 {
                  PlaceOrder(accumulate_registry[i].price_open,
                             accumulate_registry[i].volume,
                             false,
                             true,
                             0,
                             accumulate_registry[i].string_param);
                  Print("Recovered yesterday order on price ",accumulate_registry[i].price_open);
                 }
              }
           }
        }
...
//|                                                                  |
//+------------------------------------------------------------------+
   parse_result ParseString(string curr_string,int index)
     {          

      string proceed_string=curr_string;
      parse_result result;
      int init_pos          =0;
      int divide_pos        =StringFind(proceed_string,";",init_pos);
      result.OCOnum         =StringToInteger(StringSubstr(proceed_string,init_pos,divide_pos));
      init_pos              =divide_pos+1;

      divide_pos            =StringFind(proceed_string,";",init_pos);
      result.price_open     =StringToDouble(StringSubstr(proceed_string,init_pos,divide_pos));
      init_pos              =divide_pos+1;

      divide_pos            =StringFind(proceed_string,";",init_pos);
      result.volume         =StringToDouble(StringSubstr(proceed_string,init_pos,divide_pos));
      init_pos              =divide_pos+1;

      divide_pos            =StringFind(proceed_string,";",init_pos);
      result.strike_price   =StringToDouble(StringSubstr(proceed_string,init_pos,divide_pos-1));
      init_pos              =divide_pos+1;
...
//|                                                                  |
//+------------------------------------------------------------------+
   string StringGenerate(int index) //OCOnum;price;volume;time;ticket;lead_index;cover_index;strike_price
     {
      string res_string="";

      res_string     =res_string +accumulate_registry[index].OCOnum                 +";";
      res_string     =res_string +accumulate_registry[index].price_open             +";";
      res_string     =res_string +accumulate_registry[index].volume                 +";";
      //res_string     =res_string +TimeToString(accumulate_registry[index].time)     +";";
      //res_string     =res_string +accumulate_registry[index].ticket_open            +";";
      //res_string     =res_string +accumulate_registry[index].lead_order_index       +";";
      //res_string     =res_string +accumulate_registry[index].cover_order_index      +";";
      res_string=res_string+accumulate_registry[index].execution_strike_price+";";
   ...
   void RegistryRecover()
     {
      if(!PositionSelect(_Symbol))
        {return;}
      datetime p_time=PositionGetInteger(POSITION_TIME);
      HistorySelect(0,TimeCurrent());
      uint     total=HistoryOrdersTotal();
      for(int t=0;t<total;t++)
        {
         ulong ticket=HistoryOrderGetTicket(t);

         if(HistoryOrderGetString(ticket,ORDER_SYMBOL)!=_Symbol)
           {continue;}
         ENUM_ORDER_TYPE  type    =HistoryOrderGetInteger(ticket,ORDER_TYPE);
         ENUM_ORDER_STATE state   =HistoryOrderGetInteger(ticket,ORDER_STATE);
         datetime         time    =HistoryOrderGetInteger(ticket,ORDER_TIME_DONE);

         if(type==ORDER_TYPE_BUY_LIMIT && 
            state==ORDER_STATE_FILLED &&
            time>=p_time)
           {
            if(GetAccumulateIndexByPrice(HistoryOrderGetDouble(ticket,ORDER_PRICE_OPEN))!=1111){continue;}
            string c_comment=HistoryOrderGetString(ticket,ORDER_COMMENT);
            parse_result         p_result;//init
            parse_result         c_result;//cover
            if(c_comment!="")
              {
               datetime c_time   =HistoryOrderGetInteger(ticket,ORDER_TIME_SETUP);
               p_result          =ParseString(c_comment,0);

               if(IsCoverOrder(c_time,p_result,c_result))
                 {
                  double price_init   =NormalizeDouble(p_result.price_open,_Digits);
                  double volume_init  =NormalizeDouble(p_result.volume,0);
                  double strike_init  =NormalizeDouble(p_result.strike_price,_Digits);
                  datetime time_init  =TimeCurrent()-50000;

                  int res_cover=0;
                  int res_init=AddOrderToRegistry(price_init,volume_init,grid_chain_sec,grid_chain_sec,
                                                  strike_init,false,false,time_init,0,0,0,p_result.OCOnum);

                  if(res_init!=1111)
                    {
                     res_cover=AddOrderToRegistry(strike_init,volume_init,grid_chain_sec,grid_chain_sec,
                                                  price_init,true,false,time_init,0,0,0,p_result.OCOnum);

                     if(res_cover!=1111)
                       {
                        PlaceOrder(strike_init,volume_init,false,true,0,accumulate_registry[res_cover].string_param);
                        accumulate_registry[res_init].cover_order_index     =res_cover;
                        accumulate_registry[res_cover].lead_order_index     =res_init;
                       }
                     else
                       {
                        Print("Failed to add cover rec.order for price init ",price_init," for cover price ",strike_init);
                       }
                     Print("Recovered init registry order on price ",price_init);
                    }
                  else
                    {
                     Print("failed add init recover reg.order on price ",price_init);
                    }
...
//|                                                                  |
//+------------------------------------------------------------------+
   bool IsCoverOrder(datetime time_init,parse_result &parse_param,parse_result &pr_result)
     {
      HistorySelect(time_init,TimeCurrent());

      uint tot=HistoryOrdersTotal();
      for(int g=0;g<tot;g++)
        {
         ulong ticket=HistoryOrderGetTicket(g);

         if(HistoryOrderGetString(ticket,ORDER_SYMBOL)!=_Symbol)
           {continue;}
         if(HistoryOrderGetInteger(ticket,ORDER_TYPE)==ORDER_TYPE_SELL_LIMIT && 
            HistoryOrderGetInteger(ticket,ORDER_STATE)==ORDER_STATE_FILLED)
           {
            ...
            if(c_comment!="")
              {
               datetime c_time=HistoryOrderGetInteger(ticket,ORDER_TIME_SETUP);
               pr_result=ParseString(c_comment,0);

               if(pr_result.OCOnum==parse_param.OCOnum && 
                  pr_result.price_open==parse_param.strike_price)
                 {return true;break;}
              }
           }
